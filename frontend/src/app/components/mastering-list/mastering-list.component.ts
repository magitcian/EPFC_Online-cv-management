...
export class MasteringListComponent implements AfterViewInit, OnDestroy {
    ...
    dataSource: MatTableDataSource<Mastering> = new MatTableDataSource();
    filter: string = '';
    state: MatTableState;
    ...
    @Input() masterings!: Mastering[];  // masterings: Mastering[] = [];
    ...
    constructor(
        ...
        this.state = this.stateService.masteringListState;
    }
    ...
    ngAfterViewInit(): void {
        ...
        this.dataSource.filterPredicate = (data: Mastering, filter: string) => {
            const str = data.skill?.name + ' ' + data.skill?.category?.name + ' ' + data.level;
    ...
    refresh() {
        this.userService.getMasterings(this.userCvId).subscribe(masterings => {
            ...
            this.dataSource.data = masterings;
